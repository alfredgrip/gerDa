export async function GET({url}) {
    ...
    if (type === "motion") {
        ...
        const response = await compileMotion(motionParams)
        console.log(response)
        const data = {
            status: 200,
            location: response.fileName
        }
        console.log("this is from compile-motion.ts " + response.fileName)
...
async function compileMotion(params: MotionParams) {
